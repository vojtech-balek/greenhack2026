...
export interface NzuCalculatorInput {
  floorArea: number;
  footprintArea: number;
  numberOfFlats: number;
  vulnerableFlats: number;
  selectedGoals: string[];
  renovationType?: "PARTIAL" | "COMPLEX";
}

export interface NzuCalculatorResult {
  grossCapEx: number;
...
const RENOVATION_GOALS: Record<
  string,
  {
    cost: (input: { floorArea: number; footprintArea: number; numberOfFlats: number }) => number;
    savingsRate: number;
  }
> = {
  INSULATION: {
...
function validateInputs(input: NzuCalculatorInput) {
...
function calculateTotalCommercialInterest(principal: number): number {
  if (principal === 0) {
    return 0;
  }
...
function getRenovationType(input: NzuCalculatorInput): "PARTIAL" | "COMPLEX" {
...
function getStateLoanTermYears(loanAmount: number): number {
  return loanAmount <= LONG_STATE_LOAN_THRESHOLD ? SHORT_STATE_LOAN_TERM_YEARS : LONG_STATE_LOAN_TERM_YEARS;
}

function calculateVulnerableBonus(input: NzuCalculatorInput, renovationType: "PARTIAL" | "COMPLEX"): number {
...
export function calculateNzuRenovation(input: NzuCalculatorInput): NzuCalculatorResult {
  ...
  const inflatedCapEx = grossCapEx * Math.pow(1 + CAPEX_INFLATION_RATE, WAIT_YEARS);
  const penaltyCapexInflation = inflatedCapEx - grossCapEx;
  const penaltyLostSavings = Array.from({ length: WAIT_YEARS }).reduce<number>(
    (sum, _, yearIndex) => sum + estimatedYearlySavings * Math.pow(1 + ENERGY_INFLATION_RATE, yearIndex),
    0,
  );
